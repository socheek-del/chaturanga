/**
 * sg-004 evidence: screenshots of the design showcase at the two review widths, light and dark.
 *
 * Usage: node scripts/capture-design.mjs [url]   (defaults to the preview server)
 * Serve a build first: npm run build -w apps/shogi/web && npm run preview -w apps/shogi/web
 */
import { mkdirSync } from 'node:fs';
import { chromium } from '@playwright/test';

const url = process.argv[2] ?? 'http://127.0.0.1:4177/design';
...
for (const lang of ['ja', 'en']) {
  for (const width of [390, 1280]) {
    for (const scheme of ['light', 'dark']) {
      // English only at desktop width: the Japanese screenshots are the review set, English checks the Latin font.
