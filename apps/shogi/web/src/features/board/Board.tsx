...
export function Board({ theme, orientation = 'w', ...props }: BoardProps) {
  ...
  return (
    <BoardView
      ...
      renderPiece={(piece, className) => (
        <PieceSvg piece={piece as Piece} theme={theme} upsideDown={(piece as Piece).color !== orientation} className={className} />
      )}
