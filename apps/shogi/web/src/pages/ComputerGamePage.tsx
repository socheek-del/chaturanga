...
import { type Color, type PieceType, shogi } from '@chaturanga/shogi';
import { botById, BOTS } from '@chaturanga/shogi-ai';
...
const BOT_PIECE: Record<string, PieceType> = { fu: 'p', keima: 'n', gin: 's', kin: 'g', kaku: 'b', hisha: 'r' };
            ...
            return (
              ...
              >
                <span className="grid h-14 w-14 shrink-0 place-items-center rounded-2xl bg-surface-2">
                  <PieceSvg
                    piece={{ color: 'w', type: BOT_PIECE[bot.key]!, promoted: false }}
                    theme={boardTheme(boardThemeId)}
                    className="h-11 w-11"
                  />
                </span>
...
function ComputerGame({ level, humanColor }: { level: number; humanColor: Color }) {
  ...
  const askHint = async () => {
    ...
    try {
      ...
      const parsed = response.uci ? parseUci(response.uci, shogi.files) : null;
