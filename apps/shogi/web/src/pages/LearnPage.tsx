...
import type { PieceType } from '@chaturanga/shogi';
...
/** Unit bands alternate ink and seal vermilion: the "Mo" palette (apps/shogi/docs/design.md). */
...
const PIECE_TYPES = new Set<string>(['k', 'r', 'b', 'g', 's', 'n', 'l', 'p']);
...
function LessonIcon({ lesson, status }: { lesson: Lesson; status: Status }) {
  ...
  if (PIECE_TYPES.has(lesson.icon)) {
    return (
      <PieceSvg piece={{ color: 'w', type: lesson.icon as PieceType, promoted: false }} theme={boardTheme('kaya')} className="h-10 w-10" />
    );
  }
    ...
    <div className="mx-auto flex w-full max-w-xl flex-col gap-8">
      ...
      {UNITS.map((unit, u) => (
        <section key={unit.id} className="flex flex-col gap-4">
          <div data-testid="unit-banner" className={cn('motif-kaya relative w-full overflow-hidden rounded-2xl p-5 shadow-card', UNIT_STYLES[u % UNIT_STYLES.length])}>
