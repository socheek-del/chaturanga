...
test('Xiangqi links to every sibling game at its configured address, in the site language (plat-006, plat-010)', async ({ page }) => {
  expect(siblings.length).toBeGreaterThan(0);
  expect(siblings.map((site) => site.id)).not.toContain(SITE);

  await page.goto('/');
  const section = page.getByTestId('more-games');
  await expect(section.getByRole('heading')).toHaveText('更多游戏');
