...
import { Game, xiangqi } from '@chaturanga/xiangqi';
import { describe, expect, it } from 'vitest';
import { PRODUCT } from '../../../product.config';
import { ALL_LESSONS, UNITS } from './lessons';
import type { L10n } from './types';

/** Positions parse, move solutions are legal, squares answers match the engine, and zh-Hans + en are filled. */
describeLessons(PRODUCT, xiangqi, ALL_LESSONS);

const filled = (text: L10n) => PRODUCT.locales.every((lang) => (text[lang] ?? '').trim().length > 0);

describe('Xiangqi lesson content (xq-006)', () => {
  ...
  it('covers the board, every piece, the special rules and basic mates', () => {
