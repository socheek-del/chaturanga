/**
 * Xiangqi bot strength ladder (xq-003): each level must beat the level below in a majority of games.
 * Run with `npm run test:strength -w packages/xiangqi-ai`, or on GitHub Actions with
 * `gh workflow run strength.yml -f package=packages/xiangqi-ai -f pair=5 -f games=20`.
...
const outfile = join(mkdtempSync(join(tmpdir(), 'xiangqi-ladder-')), 'ladder.mjs');
